...
export default function SavedNotes() {
  ...
  useEffect(() => {
    void getAllStickyNotes()
      .then((remoteNotes: ContentStickyNote[]) => {
        setNotes(
          remoteNotes
            .slice(0, 4)
            .map((note) => ({
        ...
        {notes.length > 0 ? (
          <div className="mt-4 space-y-2">
            {notes.map((note) => (
              <div
                key={note.id}
                className="rounded-xl bg-amber-50/70 px-3 py-2"
              >
          ...
          <p className="mt-4 text-xs leading-5 text-slate-500">
            Sticky notes you make during lessons will appear here.
          </p>
        )}
      </section>
    </Link>
