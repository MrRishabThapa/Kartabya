...
import {
  buildInlineLessonSegments,
  splitLessonVisualMarkers,
} from "../../lib/lesson-visual-markers";

const visual = (id: string, position: number) => ({
  id,
  lesson_id: "lesson-1",
  content: `<p>${id}</p>`,
  position,
...
test("keeps markdown unchanged when there are no visuals", () => {
  assert.deepEqual(
    splitLessonVisualMarkers("# Lesson\n\nPlain **markdown**."),
...
test("splits one and multiple visual markers in order", () => {
  assert.deepEqual(
    splitLessonVisualMarkers("Before\n[visual]\nBetween [visual]\nAfter"),
    [
...
test("leaves visual markers inside inline code as code", () => {
  assert.deepEqual(
    splitLessonVisualMarkers("Use `[visual]` literally, then [visual]."),
    [
...
test("supports fewer or more visuals without changing marker parsing", () => {
  ...
  assert.equal(
    segments.filter((segment) => segment.type === "visual").length,
    3,
  );
  assert.equal(
    segments
      .filter((segment) => segment.type === "markdown")
      .map((segment) => (segment.type === "markdown" ? segment.content : ""))
      .join(""),
    "\n\n",
  );
});
...
test("matches visuals inline and distributes extras instead of appending them", () => {
  const segments = buildInlineLessonSegments(
    "First paragraph.\n\n[visual]\n\nSecond paragraph.",
    [visual("first", 1), visual("extra", 2)],
  );
  ...
  assert.equal(
    visualSegments[0].type === "visual" ? visualSegments[0].visual?.id : "",
    "first",
  );
  assert.equal(
    visualSegments[1].type === "visual" ? visualSegments[1].visual?.id : "",
    "extra",
  );
...
test("auto-inserts visuals when markdown has no markers", () => {
  ...
  assert.ok(
    segments.some(
      (segment) =>
        segment.type === "markdown" && segment.content.includes("Opening."),
    ),
  );
});
