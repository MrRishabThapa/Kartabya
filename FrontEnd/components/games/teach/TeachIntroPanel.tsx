...
export default function TeachIntroPanel({ onClose }: { onClose: () => void }) {
  ...
  return <AnimatePresence>
    <motion.div key="teach-backdrop" className="fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} aria-hidden="true" />
    <motion.aside key="teach-panel" role="dialog" aria-modal="true" aria-label="Teach Your Companion" className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[620px] flex-col border-l border-slate-200 bg-slate-50 shadow-2xl" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 300, damping: 32 }}>
