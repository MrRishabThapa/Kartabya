      ...
      <section className="mx-auto flex w-full max-w-[1600px] flex-col gap-4 p-4 sm:p-6 lg:h-[calc(100vh-4rem)] lg:flex-row lg:overflow-hidden">
        <article className="flex min-h-[360px] flex-1 flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] lg:min-w-0">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6 text-sm leading-7 text-slate-600 sm:px-8">
            ...
            <pre className="mt-3 overflow-x-auto rounded-xl border border-[var(--border)] bg-slate-50 p-4 font-mono text-xs leading-6 text-slate-600"><code>{`def find_first_duplicate(numbers):\n    seen = []\n    for number in numbers:\n        if number in seen:\n            return number\n        seen.append(number)\n    return None`}</code></pre>
        ...
        <section className="flex min-h-[500px] flex-1 flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-slate-50 lg:min-w-0">
          <div className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--surface)] px-4 py-3">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-700"><Code2 size={16} className="text-brand-primary" aria-hidden="true" />solution.{language === 'python' ? 'py' : 'js'}</div>
            <span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${editorLocked ? 'bg-slate-100 text-slate-500' : 'bg-emerald-50 text-emerald-700'}`}>{editorLocked ? (gameStatus === 'OVER' ? 'Game over' : 'Waiting for start') : 'Editor unlocked'}</span>
          </div>
          <div className="relative min-h-0 flex-1">
            ...
            />
            {editorLocked && <div className="pointer-events-none absolute inset-0 bg-white/30" aria-hidden="true" />}
          </div>
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] bg-[var(--surface)] px-4 py-3">
            ...
            <div className="flex gap-2">
              <button onClick={runTests} disabled={editorLocked} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[var(--border)] px-4 text-sm font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"><Play size={15} aria-hidden="true" />Run Tests</button>
          ...
          <AnimatePresence initial={false}>
            {consoleOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 150, opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-[var(--border)] bg-white"><div className="flex items-center justify-between px-4 pt-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500"><span>Test console</span><span className="text-emerald-600">● Ready</span></div><pre className="px-4 py-2 font-mono text-xs leading-6 text-slate-600">{consoleOutput}</pre></motion.div>}
          </AnimatePresence>
        </section>
      </section>

      <AnimatePresence>
        {toast && <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} role="status" className="fixed right-4 top-20 z-30 flex items-center gap-3 rounded-xl border border-amber-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-2xl"><CircleAlert size={17} className="text-amber-500" aria-hidden="true" />{toast}<button onClick={() => setToast(null)} aria-label="Dismiss notification"><X size={15} className="text-slate-500" /></button></motion.div>}
      </AnimatePresence>

      <AnimatePresence>
        {winner !== null && <motion.div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/50 p-6 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} role="dialog" aria-modal="true" aria-labelledby="duel-result-title">
          <motion.div initial={{ scale: 0.92, y: 16 }} animate={{ scale: 1, y: 0 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }} className={`w-full max-w-md rounded-3xl border bg-white p-8 text-center shadow-2xl ${winner ? 'border-emerald-200 shadow-emerald-500/10' : 'border-red-200 shadow-red-500/10'}`}>
            <div className={`mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full ${winner ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}><Trophy size={38} aria-hidden="true" /></div>
            <p className={`mb-2 text-xs font-bold uppercase tracking-[0.22em] ${winner ? 'text-emerald-600' : 'text-red-600'}`}>{winner ? 'Victory confirmed' : 'Duel concluded'}</p>
            <h2 id="duel-result-title" className={`text-3xl font-black tracking-tight ${winner ? 'text-emerald-700 [text-shadow:0_0_18px_rgba(52,211,153,0.25)]' : 'text-red-700'}`}>{winner ? 'MISSION ACCOMPLISHED' : 'SYSTEM FAILURE'}</h2>
            <p className="mt-4 text-sm leading-6 text-slate-500">{winner ? 'Your solution beat the clock and your opponent.' : 'The duel is over. Review your approach and get back in the arena.'}</p>
