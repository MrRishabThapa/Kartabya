...
import { FloatingCard, GlassPanel, GlossyOrb, GradientBlob, StackedLayers } from "@/components/marketing/visuals";
...
const features = [
  ["Adaptive lessons", "Content that adjusts to your pace, level, and the way you make sense of things."],
  ["AI tutor", "Ask anything and get a clear explanation in words that actually click for you."],
  ["Voice learning", "Speak your notes, hear your lessons, and keep learning with your hands free."],
  ["Smart sticky notes", "Capture the good stuff as you learn. Adaptiv keeps every idea organized for you."],
  ["Progress tracking", "See your momentum grow with beautiful analytics built around real understanding."],
  ["Multi-language", "Learn in English, Nepali, and more with support that meets you where you are."],
] as const;
...
const steps = [
  ["01", "Tell us your goals", "A few thoughtful questions help us understand what you want to learn and why it matters."],
  ["02", "Start your way", "Learn through lessons, voice, notes, and conversations that fit your natural rhythm."],
  ["03", "Grow with your tutor", "Your AI tutor remembers what helps and adapts every next step around your progress."],
] as const;
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(36,50,71,0.14)]">
        ...
        <div className="grid min-h-[300px] grid-cols-[56px_1fr] sm:grid-cols-[76px_1fr]">
          <aside className="border-r border-slate-100 bg-slate-50/50 p-3">
            ...
            {["01", "02", "03", "04"].map((item, index) => <div key={item} className={`mb-3 grid h-9 w-9 place-items-center rounded-xl text-[9px] font-bold ${index === 1 ? "bg-brand-primary-bg text-brand-primary" : "text-slate-300"}`}>{item}</div>)}
          </aside>
          <div className="bg-[#fffdfa] p-4 sm:p-7">
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_210px]">
              ...
              <div className="rounded-2xl bg-slate-800 p-4 text-white"><div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-brand-primary-light" /><span className="text-xs font-bold">AI tutor</span></div><p className="mt-4 text-xs leading-5 text-slate-300">Want me to explain that with an example?</p><div className="mt-5 flex items-center gap-2 rounded-lg bg-white/10 px-2.5 py-2 text-[10px] text-slate-400"><span className="h-1.5 w-1.5 rounded-full bg-brand-primary-light" />Ask a follow-up...</div></div>
            </div>
    ...
    <main className="landing-page overflow-hidden bg-[#FEF2EA] text-slate-800">
      ...
      <section className="relative isolate flex min-h-[820px] items-center px-5 pb-16 pt-36 sm:px-8 lg:min-h-screen lg:px-12 lg:pt-32" style={{ backgroundImage: `radial-gradient(circle at ${spotlight.x}% ${spotlight.y}%, rgba(242,121,40,0.13), transparent 30%)` }}>
        ...
        <div className="mx-auto grid w-full max-w-7xl items-center gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-10">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.25, ease: [0.22, 1, 0.36, 1] }} className="relative lg:pt-10"><ProductMockup /><div className="absolute -bottom-8 -left-7 hidden items-center gap-3 rounded-2xl border border-white bg-white/90 p-3 shadow-lg backdrop-blur sm:flex"><div className="grid h-9 w-9 place-items-center rounded-xl bg-brand-primary-bg text-brand-primary"><span className="h-2 w-2 rounded-full bg-brand-primary" /></div><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Your next win</p><p className="text-sm font-extrabold text-slate-700">15 min of focused learning</p></div></div></motion.div>
        </div>
      </section>
      ...
      <section id="features" className="relative bg-[#fffaf6] px-5 py-24 sm:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><Reveal><p className="text-sm font-bold uppercase tracking-[0.16em] text-brand-primary">Why Adaptiv</p><h2 className="mt-4 max-w-2xl text-4xl font-extrabold leading-tight tracking-[-0.05em] text-slate-800 sm:text-5xl">A learning system that<br /><span className="text-brand-primary">learns you back.</span></h2><p className="mt-5 max-w-xl text-lg leading-8 text-slate-500">Every learner is different. Adaptiv turns that difference into your advantage.</p></Reveal><div className="mt-16 space-y-6 lg:space-y-8">{features.map(([title, description], index) => <Reveal key={title} delay={index * 0.06}><article className={`grid items-center gap-8 overflow-hidden rounded-[2rem] border border-[#ead8cc] bg-white p-5 shadow-[0_20px_60px_-30px_rgba(194,97,32,0.28)] sm:p-8 lg:grid-cols-2 lg:gap-16 lg:p-10 ${index % 2 === 1 ? "lg:[&>div:first-child]:order-2" : ""}`}><div className="relative min-h-64 overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-[#fffaf6] via-[#fbe0cc] to-[#f6b27d] p-6 sm:min-h-72">{index === 0 && <><GradientBlob className="absolute -right-20 -top-12 h-[125%] w-[125%]" /><FloatingCard className="absolute bottom-7 left-7 w-64 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-brand-primary">Adaptive lesson</p><p className="mt-3 text-xl font-extrabold tracking-tight text-slate-800">A pace that feels like yours.</p><div className="mt-5 h-2 rounded-full bg-brand-primary-tint"><div className="h-full w-3/5 rounded-full bg-brand-primary" /></div></FloatingCard></>}{index === 1 && <><GlossyOrb className="absolute -right-5 top-4 h-32 w-32" /><GlassPanel className="absolute bottom-7 left-7 max-w-[260px] p-5"><p className="text-xs font-bold text-brand-primary">Tutor note</p><p className="mt-3 text-sm leading-6 text-slate-600">“Try thinking of it as a conversation, not a test.”</p><div className="mt-4 h-1.5 w-20 rounded-full bg-brand-primary/40" /></GlassPanel><div className="absolute right-8 top-10 h-20 w-20 rounded-[1.5rem] border border-white/70 bg-white/35 backdrop-blur" /></>}{index === 2 && <><StackedLayers className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" /><div className="absolute bottom-5 right-6 text-right"><p className="text-[10px] font-bold uppercase tracking-wider text-brand-primary-dark">Learn out loud</p><p className="text-xs font-semibold text-slate-500">Voice notes, hands free</p></div></>}{index === 3 && <><div className="absolute inset-8 rotate-[-6deg] rounded-[1.5rem] bg-[#ffe8a8] p-5 shadow-[0_20px_40px_-20px_rgba(157,79,26,0.4)]"><p className="text-xs font-bold text-amber-700">Remember this</p><p className="mt-4 max-w-[13rem] text-lg font-extrabold leading-tight text-slate-800">The best ideas deserve a place to land.</p><div className="absolute bottom-5 right-5 h-3 w-3 rounded-full bg-amber-400" /></div></>}{index === 4 && <><div className="absolute inset-x-8 bottom-8 top-10 rounded-[1.5rem] border border-white/80 bg-white/50 p-5 backdrop-blur"><div className="flex items-end gap-2"><div className="h-16 w-5 rounded-t-full bg-brand-primary/35" /><div className="h-24 w-5 rounded-t-full bg-brand-primary/55" /><div className="h-20 w-5 rounded-t-full bg-brand-primary/45" /><div className="h-32 w-5 rounded-t-full bg-brand-primary" /><div className="h-28 w-5 rounded-t-full bg-brand-primary/80" /></div><div className="mt-5 flex justify-between text-[10px] font-bold uppercase tracking-wider text-slate-400"><span>Last week</span><span className="text-brand-primary">This week +24%</span></div></div></>}{index === 5 && <><div className="absolute left-8 top-10 text-[5rem] font-extrabold leading-none tracking-[-0.12em] text-brand-primary/25">Aa</div><div className="absolute bottom-10 right-8 rounded-2xl border border-white/80 bg-white/65 p-4 shadow-lg backdrop-blur"><span className="text-2xl font-extrabold text-slate-800">नेपाली</span><span className="ml-2 text-sm font-semibold text-brand-primary">+ English</span></div></>}</div><div className="lg:px-2"><p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-primary">0{index + 1} / made personal</p><h3 className="mt-4 text-3xl font-extrabold tracking-[-0.05em] text-slate-800 sm:text-4xl">{title}</h3><p className="mt-4 max-w-md text-base leading-7 text-slate-500">{description}</p><a href="#how-it-works" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-brand-primary hover:text-brand-primary-dark">Learn more <ArrowRight size={15} /></a></div></article></Reveal>)}</div></div></section>

      <section id="how-it-works" className="relative px-5 py-24 sm:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><Reveal className="text-center"><p className="text-sm font-bold uppercase tracking-[0.16em] text-brand-primary">How it works</p><h2 className="mt-4 text-4xl font-extrabold tracking-[-0.05em] text-slate-800 sm:text-5xl">Start where you are.<br /><span className="text-slate-400">Go farther from there.</span></h2></Reveal><div className="mt-16 grid gap-5 lg:grid-cols-3">{steps.map(([number, title, description], index) => <Reveal key={title} delay={index * 0.1}><article className="relative min-h-[340px] overflow-hidden rounded-[2rem] border border-[#ead8cc] bg-[#fffaf6] p-7 shadow-sm"><p className="text-[6rem] font-extrabold leading-[0.8] tracking-[-0.12em] text-brand-primary/20">{number}</p><div className="absolute -right-12 -top-8 h-40 w-40 rounded-full bg-brand-primary/10 blur-2xl" />{index === 0 && <div className="absolute bottom-7 right-7 h-24 w-32 rotate-[-6deg] rounded-2xl border border-white bg-white/75 p-3 shadow-lg"><div className="h-2 w-14 rounded-full bg-brand-primary/50" /><div className="mt-4 h-2 w-20 rounded-full bg-slate-100" /><div className="mt-2 h-2 w-16 rounded-full bg-slate-100" /></div>}{index === 1 && <GlassPanel className="absolute bottom-7 right-7 w-36 rotate-[4deg] p-4"><div className="h-2 w-16 rounded-full bg-brand-primary/50" /><div className="mt-3 text-xs font-bold text-slate-700">Choose your pace</div><div className="mt-3 h-1.5 rounded-full bg-brand-primary-tint"><div className="h-full w-2/3 rounded-full bg-brand-primary" /></div></GlassPanel>}{index === 2 && <GlossyOrb className="absolute bottom-7 right-8 h-24 w-24" />}<div className="relative mt-12"><h3 className="text-xl font-extrabold tracking-tight text-slate-800">{title}</h3><p className="mt-3 max-w-xs text-sm leading-6 text-slate-500">{description}</p></div></article></Reveal>)}</div></div></section>
