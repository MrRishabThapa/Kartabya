  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="report-title">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        {submitted ? <div className="mt-6 rounded-xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-700">Report submitted. Thank you for helping keep the community safe.</div> : <><div className="mt-6 space-y-2">{reasons.map((reason) => <label key={reason} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 text-sm font-medium transition-colors ${selected === reason ? 'border-violet-300 bg-violet-50 text-violet-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}><input type="radio" name="reason" checked={selected === reason} onChange={() => setSelected(reason)} className="accent-violet-600" />{reason}</label>)}</div><button onClick={() => setSubmitted(true)} className="mt-6 w-full rounded-xl bg-red-600 px-4 py-3 text-sm font-bold text-white transition-colors hover:bg-red-700">Submit report</button></>}
      </div>
    </div>
  );
}
