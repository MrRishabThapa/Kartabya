...
export default function VoteControl({ score, vote, onVote, compact = false }: VoteControlProps) {
  return (
    <div className={`inline-flex items-center rounded-lg bg-slate-100 p-0.5 ${compact ? 'gap-0.5' : 'gap-1'}`}>
      ...
      <button aria-label="Downvote" onClick={() => onVote('down')} className={`rounded-md p-1 transition-colors ${vote === 'down' ? 'bg-slate-300 text-slate-800' : 'text-slate-500 hover:bg-white hover:text-slate-800'}`}>
